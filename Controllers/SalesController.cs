...
namespace PharmacyManagement.Controllers
{
    public class SalesController : Controller
    {
        ...
        public ActionResult SalesInvoiceTemplate(SalesModel model)
        {
            var customerMedicinePurchased = getMedicinePurchased(model.CustomerPurchase.CustomerPurchaseId);
            if(customerMedicinePurchased != null)
            {
                model.CustomerPurchasedMedicine = customerMedicinePurchased;
        ...
        public List<CustomerPurchasedMedicine> getMedicinePurchased(int CustomerPurchaseId)
        {
            try
            {
                List<CustomerPurchasedMedicine> medicineList = new List<CustomerPurchasedMedicine>();
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    conn.Open();
                    string medcinePurchasequery = "Select mp.CustomerPurchasedMedicineId,mp.MedicineId, m.MedicineName,mp.CustomerPurchasedId, mp.Quantity,mp.Rate,mp.Amount,mp.DeletedFlag, m.PackingType from CustomerPurchasedMedicine mp join Medicines m on mp.MedicineId=m.MedicineId where mp.DeletedFlag='N' and mp.CustomerPurchasedId=" + CustomerPurchaseId;
                    using (SqlCommand cmd = new SqlCommand(medcinePurchasequery, conn))
                    {
                        cmd.CommandType = CommandType.Text;
                        var rdr = cmd.ExecuteReader();
                        while (rdr.Read())
                        {
                            ...
                            customerPurchasedMedicine.CustomerPurchasedMedicineId = int.Parse(rdr[0].ToString());
                            customerPurchasedMedicine.MedicineId = int.Parse(rdr[1].ToString());
                            customerPurchasedMedicine.MedicineName = rdr[2].ToString();
                            customerPurchasedMedicine.CustomerPurchaseId = int.Parse(rdr[3].ToString());
                            customerPurchasedMedicine.Quantity = int.Parse(rdr[4].ToString());
                            customerPurchasedMedicine.Price = decimal.Parse(rdr[5].ToString());
                            customerPurchasedMedicine.TotalAmount = decimal.Parse(rdr[6].ToString());
                            customerPurchasedMedicine.DeletedFlag = char.Parse(rdr[7].ToString());
                            customerPurchasedMedicine.PackingType = rdr[8].ToString();
                            medicineList.Add(customerPurchasedMedicine);
                        }
                    }
                    return medicineList;
